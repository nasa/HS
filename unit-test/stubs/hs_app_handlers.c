 ...
 *
 *-----------------------------------------------------------------*/
void UT_DefaultHandler_HS_ComputeStatusFlags(void *UserObj, UT_EntryKey_t FuncKey, const UT_StubContext_t *Context)
{
    uint8       *StatusFlagsOut = UT_Hook_GetArgValueByName(Context, "StatusFlagsOut", uint8 *);
    const size_t ExpectedSize   = sizeof(*StatusFlagsOut);

    if (StatusFlagsOut)
    {
        if (UT_Stub_CopyToLocal(FuncKey, StatusFlagsOut, ExpectedSize) < ExpectedSize)
        {
            memset(StatusFlagsOut, 0, ExpectedSize);
        }
    }
}
