...
for index = 1 to HS_MAX_EXEC_CNT_SLOTS do
  if (SCX_CPU1_HS_XCT[index].ResourceName = "") then
    ...
    if (SCX_CPU1_HS_ExecutionCtr[index] > 0) AND ;;
       (%hex(SCX_CPU1_HS_ExecutionCtr[index]) < %hex(x'FFFFFFFF')) then
      write "<*> Passed (7100) - Execution Counter for '",SCX_CPU1_HS_XCT[index].ResourceName, "' = ",SCX_CPU1_HS_ExecutionCtr[index]
    else
      write "<!> Failed (7100) - Execution counter for entry ",index," is not within the proper range. Actual value = ",SCX_CPU1_HS_ExecutionCtr[index] 
    endif
  endif
enddo
...
ut_setupevents "SCX", "CPU1", {HSAppName}, HS_XCTVAL_INF_EID, "INFO", 2
...
if (SCX_CPU1_HS_ExecutionCtr[6] = 0xFFFFFFFF) AND ;;
   (SCX_CPU1_HS_ExecutionCtr[7] = 0xFFFFFFFF) then
...
for index = 1 to HS_MAX_EXEC_CNT_SLOTS do
  if (SCX_CPU1_HS_ExecutionCtr[index] <> 0xFFFFFFFF) then
    break
  endif
enddo
...
ut_setupevents "SCX", "CPU1", {HSAppName}, HS_XCTVAL_INF_EID, "INFO", 2
ut_setupevents "SCX", "CPU1", {HSAppName}, HS_XCTVAL_ERR_EID, "ERROR", 3
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_hs_exectr "
write ";*********************************************************************"
ENDPROC
